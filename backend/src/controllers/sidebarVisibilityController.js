...
const createDefaultSidebarConfig = () => ({
  ADMIN: ['dashboard', 'staffManagement', 'patientManagement', 'serviceManagement', 'reports', 'medicalClinic', 'doctorPerformance', 'billingReport', 'labReports', 'radiologyReports', 'nurseReport', 'pharmacyReport', 'diseaseReports', 'diseaseTally', 'ageGenderDisease', 'diseaseManagement', 'abortionCare', 'familyPlanning', 'centralRegister', 'patientAccounts', 'bedManagement', 'cardProducts', 'doctorCommissions', 'auditLogs', 'loanApproval', 'systemView', 'patientRegistration', 'admissions'],
  DOCTOR: ['dashboard', 'patientQueue', 'dailyWork', 'abortionCare', 'familyPlanning', 'admissions', 'patientHistory', 'medicalCertificate', 'internationalCertificate', 'referPatient', 'appointments', 'loans', 'patientRegistration'],
  NURSE: ['dashboard', 'triageQueue', 'familyPlanning', 'abortionCare', 'admissions', 'dailyTasks', 'walkInServices', 'walkInOrders', 'continuousVitals', 'gallery', 'appointments', 'loans', 'patientRegistration'],
...
exports.getSidebarVisibility = async (req, res) => {
  try {
    ...
    try {
      ...
      // Auto-merge: add any keys from defaults that are missing in the stored config
      let merged = false;
      Object.keys(defaultConfig).forEach(role => {
        if (!config[role]) {
          config[role] = defaultConfig[role];
          merged = true;
        } else {
          const missing = defaultConfig[role].filter(k => !config[role].includes(k));
          if (missing.length > 0) {
            config[role] = [...config[role], ...missing];
            merged = true;
          }
        }
      });
