        ...
        <div className="card mb-6 max-h-[80vh] flex flex-col">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1">
            ...
            <div className="border-t-2 border-gray-300 pt-4 mb-6">
              <h3 className="font-medium mb-3 text-gray-800">Outcome</h3>
            ...
            <div className="border-t-2 border-gray-200 pt-5 mt-2 mb-6">
              <h3 className="font-medium mb-3 text-gray-800">Service Provider</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
            ...
            <div className="shrink-0 border-t-2 border-gray-200 bg-white px-6 py-5 flex justify-end gap-3 mt-2">
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-100">
            <tr className="border-b-2 border-gray-300">
              ...
              <th className="p-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">MRN</th>
              <th className="p-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Age/Gest.</th>
              <th className="p-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Care Type</th>
              <th className="p-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Procedure</th>
              <th className="p-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">G/P</th>
              <th className="p-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">HIV</th>
              <th className="p-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Outcome</th>
              <th className="p-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Actions</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map(r => (
              <tr key={r.id} className="border-b border-gray-200 hover:bg-gray-50 even:bg-gray-50">
                <td className="p-4 text-sm">{new Date(r.createdAt).toLocaleDateString()}</td>
                <td className="p-4">
                  <div className="font-medium text-sm">{r.mrn || r.patientId}</div>
                </td>
                <td className="p-4 text-sm">{r.age || '-'} / {r.gestationalAgeWeeks ? `${r.gestationalAgeWeeks}w` : '-'}</td>
                <td className="p-4 text-sm">
                  <span className={`badge ${r.careType === 'SAFE_ABORTION' ? 'badge-warning' : 'badge-info'}`}>
                    {r.careType === 'SAFE_ABORTION' ? 'Safe' : 'Post-Abortion'}
                  </span>
                </td>
                <td className="p-4 text-sm">{PROCEDURE_TYPES.find(p => p.value === r.procedureType)?.label?.split(' ')[0] || '-'}</td>
                <td className="p-4 text-sm">{r.gravida}/{r.para}</td>
                <td className="p-4 text-sm">
